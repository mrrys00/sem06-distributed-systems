...
@ray.remote
def bubble_sort_distributed(arr: list()):
    return bubble_sort(arr)

arr = [64, 34, 25, 12, 22, 11, 90]
def run_bubb_remote(arr):
    result = ray.get([bubble_sort_distributed.remote(arr) for _ in range(os.cpu_count())])
    print("eee ", arr)
    return result

print('remote')
cProfile.run("run_bubb_remote(arr)")
